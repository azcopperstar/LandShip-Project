...
struct EditSubscriptions: View {
	...
	@State private var lastPayment = Date()
	...
	@State private var itemRecurring: Bool = false
	@State private var itemRecurringInterval: String = "Month"
	@State private var itemRecurringDays: Int = 0
...
//	@State private var selectedPart5: MxParts1? = nil
	...
	init(Records: Subscriptions, startEditing: Bool = false) {
		...
		self._lastPayment = State.init(initialValue: dataSet.lastPayment)
		...
		self._accountNumber = State.init(initialValue: dataSet.accountNumber)
		self._category = State.init(initialValue: dataSet.category)
		self._subCategory = State.init(initialValue: dataSet.subCategory)
		self._itemRecurring = State.init(initialValue: dataSet.itemRecurring)
		self._itemRecurringInterval = State.init(initialValue: dataSet.itemRecurringInterval)
		self._itemRecurringDays = State.init(initialValue: dataSet.itemRecurringDays)
			...
			ScrollView {
				...
				CardView {
					VStack {
						...
						// Payment date
						HStack{LabelDataTextview(label: "Expense Name", data: $itemName)}
						HStack{LabelDataPicker_Date(label: "Last Payment Due", data: $lastPayment)}
						HStack{LabelDataTextview(label: "Account Number", data: $accountNumber)}

						HStack{LabelDataToggle(label: "Recurring Expense?", data: $itemRecurring)}
						if itemRecurring{
							// Interval quantity
							HStack{LabelDataTextview_Numberpad_Int(label: "Recurring Every", data: $itemRecurringDays)}
								.onChange(of: itemRecurringDays) { _, newValue in
									dataSet.itemRecurringDays = newValue
								}
							// Interval unit picker
							HStack {
								Text("Interval Period")
									.textLabelModified()
								Picker("Interval Period", selection: $itemRecurringInterval) {
									Text("Day").tag("Day")
								...
								.frame(maxWidth: .infinity, alignment: .trailing)
								.onChange(of: itemRecurringInterval) { _, newValue in
									dataSet.itemRecurringInterval = newValue
								}
							}
						}
						
						// Inserted new HStack showing next payment due date when recurring
						if itemRecurring {
							HStack {
								Text("Next Payment Due")
									.textLabelModified()
								Spacer()
								Text(functions.formatDate_DDMMMyy(date: nextPaymentDue))
									.frame(maxWidth: .infinity, alignment: .trailing)
							}
						}
						
						HStack{LabelDataTextview_Numberpad_Currency(label: "Expense Cost", data: $itemCost)}
				...
				CardView {
					VStack {
						SectionText(label: "EXPENSE DETAILS")
						
						// Manual overrides for the service item and descriptions
//						TextFieldNote_FullWidth_3lines(sectionText: "Expense Name", prompt: "Enter Item...", data: $itemName)

						// Multiline Description
						VStack(alignment: .leading, spacing: 6) {
							TextFieldNote_FullWidth_3lines(sectionText: "Expense Description", prompt: "Enter Description...", data: $itemDescription)

							TextFieldNote_FullWidth_3lines(sectionText: "Expense Notes", prompt: "Enter Notes...", data: $itemNotes)
					...
					VStack {
						...
						CardView {
							VStack {
								...
								HStack{LabelDataText(label: "Sub-Category", data: dataSet.subCategory)}
							}}
						CardView {
							VStack {
								
								HStack{LabelDataCurrency(label: "Expense Cost", data: Float(itemCost), unit: "")}
								if dataSet.itemRecurring {
									//							HStack{LabelDataText(label: "Recurring Expense ", data: "YES")}
									if dataSet.itemRecurringDays == 1 {
										HStack{LabelDataText(label: "Recurring Every", data: "\(dataSet.itemRecurringDays) \(dataSet.itemRecurringInterval)")
										}
									} else {
										HStack{LabelDataText(label: "Recurring Every", data: "\(dataSet.itemRecurringDays) \(dataSet.itemRecurringInterval)'s")
										}
									}
								}
								HStack{LabelDataText(label: "Last Payment", data: "\(functions.formatDate_DDMMMyy(date: dataSet.lastPayment))")}
								if dataSet.itemRecurring {
									HStack{LabelDataText(label: "Next Payment Due", data: functions.formatDate_DDMMMyy(date: nextPaymentDue))}
								}
...
//				loadItemIntervalsIfNeeded()
	...
	private func updateItem() {
		...
		dataSet.updatedAt = Date()
		dataSet.lastPayment = lastPayment
		...
		dataSet.accountNumber = accountNumber
		dataSet.category = category
		dataSet.subCategory = subCategory
		dataSet.itemRecurring = itemRecurring
		dataSet.itemRecurringInterval = itemRecurringInterval
		dataSet.itemRecurringDays = itemRecurringDays
...
//		}
    ...
    private func loadAllCategories() {
        let fd = FetchDescriptor<Subscriptions>()
        do {
	...
	// Computes the next payment due date based on lastPayment, interval unit and quantity.
	private var nextPaymentDue: Date {
	    let calendar = Calendar.current
	    let now = Date()
	    let base = lastPayment
	    let qty = max(0, itemRecurringDays)
	    // If not recurring or qty is zero, just return last payment as fallback
	    guard itemRecurring, qty > 0 else { return base }

	    // Build the interval components once
	    var comps = DateComponents()
	    switch itemRecurringInterval {
	    case "Day":
	        comps.day = qty
	    case "Month":
	        comps.month = qty
	    ...
	    // Start from the first due date after the last payment
	    var candidate = calendar.date(byAdding: comps, to: base) ?? base

	    // If candidate is in the past or equal to now, keep adding the interval until it's in the future
	    while candidate <= now {
	        if let next = calendar.date(byAdding: comps, to: candidate) {
	            candidate = next
	        } else {
	            // Fallback: break to avoid infinite loop due to calendar failure
	            break
	        }
	    }
	    return candidate
