...
struct pdfReportSubscriptions: View {
	...
	@Query private var dataSet: [Subscriptions]
...
//	@Binding var trackVehicleSelected: String
	...
	private var dataSetKey: String {
		let sorted = dataSet.sorted { (lhs: Subscriptions, rhs: Subscriptions) -> Bool in
	...
	init(trackVehicleSelected: String) {
		...
		let sortDescriptors: [SortDescriptor<Subscriptions>] = [
		...
		if vehicleId != "All Vehicles" {
			...
			let id = vehicleId
			let predicate: Predicate<Subscriptions> = #Predicate { $0.vehicleId == id }
			self._dataSet = Query<Subscriptions, [Subscriptions]>(
				filter: predicate,
	...
	private func generateAndShowPDF() {
        ...
        if let doc = PDFDocument(data: pdfData) {
            ...
            lastPDFHash = hash
            _ = savePDF(data: pdfData, fileName: "Expenditures")
        }
	}
	...
	// Produce the 8 column strings for an additions record (in the same order as the table columns).
	// Columns: DATE, TITLE, DESCRIPTION, NOTES, VENDOR, COST, MILES, IMAGE
	func valuesForRecord(_ record: Subscriptions) -> [String] {
		let date = functions.formatDate_DDMMM_yyyy(date: record.createdAt)
		let title = record.itemName
		let miles = String(record.miles)
		let vendor = record.itemVendor
		let description = record.itemDescription
		let imagePlaceholder = record.image1 != nil ? "(image)" : ""
        // Build cost column with optional recurring details
        var cost = functions.formatCurrency(dollars: max(0, record.itemCost))
        if record.itemRecurring {
            let interval = record.itemRecurringInterval
            let days = record.itemRecurringDays
            if !interval.isEmpty && days > 0 {
                cost += "\n(Recurring — Interval: \(interval), Days: \(days))"
		...
		return [date, title, description, record.itemNotes, vendor, cost, miles, imagePlaceholder]
	}
	...
	func generatePDFWithTable(units: [String]) -> Data? {
		...
		// Table header labels for additions report (8 columns).
		let headerRows: [[String]] = [
			["DATE", "EXPENDITURE", "DESCRIPTION", "NOTES", "VENDOR", "COST", "MILES", "IMAGE"]
		]
		...
		// 8 column widths as a fraction of content width (sum approximately 1.0).
		...
		let reportTitle = "Expenditures, Subscriptions and Recurring Payments"
		...
		for (index, record) in dataSet.enumerated() {
			...
			if let cs = currentSubcategory, cs != newSubcategory {
				...
				currentY += drawTotalsRow(label: "Subtotal — \(cs)", amount: subcategoryTotal, at: CGPoint(x: margin, y: currentY), columnWidths: columnWidths, pageHeight: pageHeight)
				subcategoryTotal = 0
			}
			...
			// Add to totals
			vehicleTotal += record.itemCost
			subcategoryTotal += record.itemCost
			categoryTotal += record.itemCost
			grandTotal += record.itemCost
		...
		let data = pdfRenderer.pdfData { context in
			...
			for (index, record) in dataSet.enumerated() {
				...
				if let cs = currentSubcategory, cs != newSubcategory {
					...
					currentY += drawTotalsRow(label: "Subtotal — \(cs)", amount: subcategoryTotal, at: CGPoint(x: margin, y: currentY), columnWidths: columnWidths)
					subcategoryTotal = 0
				}
				// If category changes and it's not the first row, emit category subtotal
				if let cc = currentCategory, cc != newCategory {
	...
	func computeRowHeight(for record: Subscriptions, columnWidths: [CGFloat], minRowHeight: CGFloat) -> CGFloat {
		...
		// Build per‑column paragraph styles: left for TITLE, DESCRIPTION, NOTES, and VENDOR; right for COST and MILES; center otherwise.
		var paragraphStyles: [NSMutableParagraphStyle] = []
		for i in 0..<values.count {
			let p = NSMutableParagraphStyle()
			p.lineBreakMode = .byWordWrapping
			switch i {
			case titleIndex, descriptionIndex, notesIndex, 4: // TITLE, DESCRIPTION, NOTES, VENDOR
				p.alignment = .left
			case 5, 6: // COST, MILES
				p.alignment = .right
			default:
	...
	// macOS variant converts to Quartz coordinates; iOS draws directly.
	func drawTableRow(record: Subscriptions, at origin: CGPoint, columnWidths: [CGFloat], rowHeight: CGFloat, rowIndex: Int, pageHeight: CGFloat? = nil) {
		let values = valuesForRecord(record)
		let titleIndex = 1
		let descriptionIndex = 2
		let notesIndex = 3
	...
	func drawTotalsRow(label: String, amount: Float, at origin: CGPoint, columnWidths: [CGFloat], pageHeight: CGFloat? = nil) -> CGFloat {
	    ...
	    // Amount in COST column (index 5)
	    let costX = origin.x + columnWidths[0] + columnWidths[1] + columnWidths[2] + columnWidths[3] + columnWidths[4]
	    let costWidth = columnWidths[5]
	...
	private func saveAsPDF() {
		...
		let dateStamp = DateFormatter.localizedString(from: Date(), dateStyle: .short, timeStyle: .short)
			...
			.replacingOccurrences(of: ":", with: "-")
			.replacingOccurrences(of: ",", with: "")
		panel.nameFieldStringValue = "Subscriptions - \(vehicleTitle) - \(dateStamp).pdf"
	...
	private func sharePDFiOS() {
		...
		let dateStamp = DateFormatter.localizedString(from: Date(), dateStyle: .short, timeStyle: .short)
			...
			.replacingOccurrences(of: ":", with: "-")
			.replacingOccurrences(of: ",", with: "")
		let fileName = "Subscriptions - \(vehicleTitle) - \(dateStamp).pdf"
	...
	private func printPDF() {
		...
		printInfo.jobName = "Subscriptions"
...
#Preview {
    ...
    let container = try! ModelContainer(for: Subscriptions.self, Settings1.self, configurations: config)

    // Seed sample subscriptions data for visual verification of layout, grouping, and totals.
    let context = container.mainContext
    func makeSubscription(category: String, subCategory: String, daysAgo: Int, vehicleId: String, name: String, cost: Float, notes: String) -> Subscriptions {
    ...
    let samples: [Subscriptions] = [
        makeSubscription(category: "Engine",     subCategory: "Oil",      daysAgo: 0,  vehicleId: "Vehicle A", name: "Oil Change",         cost: 120,   notes: "Next change in 3 months."),
        makeSubscription(category: "Engine",     subCategory: "Oil",      daysAgo: 10, vehicleId: "Vehicle A", name: "Oil Filter",         cost: 18.5,  notes: "OEM filter."),
        makeSubscription(category: "Engine",     subCategory: "Cooling",  daysAgo: 20, vehicleId: "Vehicle A", name: "Coolant Flush",       cost: 95,    notes: "Replaced coolant."),
        makeSubscription(category: "Brakes",     subCategory: "Front",    daysAgo: 15, vehicleId: "Vehicle A", name: "Front Pads",          cost: 210,   notes: "Resurfaced rotors."),
        makeSubscription(category: "Brakes",     subCategory: "Rear",     daysAgo: 18, vehicleId: "Vehicle A", name: "Rear Pads",           cost: 180,   notes: "New pads."),
        makeSubscription(category: "Electrical", subCategory: "Battery",  daysAgo: 5,  vehicleId: "Vehicle B", name: "Battery Replacement", cost: 170,   notes: "Starts faster."),
        makeSubscription(category: "Electrical", subCategory: "Lighting", daysAgo: 2,  vehicleId: "Vehicle B", name: "Headlight Bulb",      cost: 25,    notes: "Left bulb replaced."),
    ]
