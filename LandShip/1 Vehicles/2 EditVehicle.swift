...
struct EditVehicle: View {
	...
	init(dataSet: Vehicle8, trackVehicleSelected: Binding<String>) {
			...
			ScrollView {
				CardView {
					VStack {
						...
						HStack{LabelDataToggle(label: "Deactivate Vehicle       ", data: $inactive)}
						Text("When enabled, this vehicle is marked as inactive. It will be hidden in lists and pickers, but its data remains available for viewing and editing, and can be un-hidden by selecting 'View Inactive' on the 'Settings' screen.")
							.font(.caption)
			...
			.toolbar {
				...
				ToolbarItem(placement: .automatic) {
					...
					.confirmationDialog("Confirm action", isPresented: $isPresentingConfirm) {
						...
						Text("Confirm deletion of this vehicle record...")
					}
