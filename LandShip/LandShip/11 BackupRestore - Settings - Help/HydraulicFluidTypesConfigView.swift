// HydraulicFluidTypesConfigView.swift
//
// Lets the user choose which hydraulic fluid types appear in the Hydraulic Fluid Type
// picker on Edit Vehicle. Aviation vertical (MIL-PRF-5606, Skydrol, HyJet, ...).
// Saved to the CloudKit-synced Settings1 singleton, so the selection follows the user
// across devices — mirrors FuelTypesConfigView.

import SwiftUI
import SwiftData

struct HydraulicFluidTypesConfigView: View {
	...
	@State private var enabledFluids: Set<AviationHydraulicFluidType> = Set(AviationHydraulicFluidType.defaultEnabled)
	...
	private func loadFromSettings() {
		...
		enabledFluids = Set(s.enabledHydraulicFluidTypes)
	}

	private func toggle(_ fluid: AviationHydraulicFluidType) {
	...
	private func restoreDefaults() {
		enabledFluids = Set(AviationHydraulicFluidType.defaultEnabled)
	}

	private func save() {
		guard let s = settings else { return }
		// Keep catalog declaration order within each category rather than Set's arbitrary order.
		s.enabledHydraulicFluidTypes = AviationHydraulicFluidType.allCases.filter { enabledFluids.contains($0) }
		do { try modelContext.save() } catch { print("Failed to save hydraulic fluid type scheme: \(error)") }
	}

	@ViewBuilder
	private func fluidRow(_ fluid: AviationHydraulicFluidType) -> some View {
		Button {
			toggle(fluid)
		} label: {
			HStack {
	...
	private var content: some View {
		List {
			Section {
				Text("Choose which hydraulic fluid types appear in the Hydraulic Fluid Type picker when editing an \(Vertical.current.assetSingular.lowercased()). This is saved to your account and applies on every device.")
					.font(.caption)
					.foregroundStyle(.secondary)
			}
			...
			ForEach(AviationHydraulicFluidType.Category.allCases, id: \.self) { category in
				Section(category.rawValue) {
					ForEach(AviationHydraulicFluidType.allCases.filter { $0.category == category }) { fluid in
						fluidRow(fluid)
					}
				}
			}
...
#Preview {
	...
	return HydraulicFluidTypesConfigView()
