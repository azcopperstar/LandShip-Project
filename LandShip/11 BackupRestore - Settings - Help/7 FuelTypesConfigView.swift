// FuelTypesConfigView.swift
//
// Lets the user choose which fuel types appear in the Fuel Type picker on Edit
// Vehicle / Edit Fuel Log. Only relevant to the aviation vertical, which has dozens
// of named fuels (100LL, Jet A, JP-8, SAF, ...) versus land/marine's fixed four-option
// list. Saved to the CloudKit-synced Settings1 singleton, so the selection follows
// the user across devices — mirrors the persistence pattern used for the dashboard
// card scheme in DashboardConfigView.

import SwiftUI
import SwiftData

struct FuelTypesConfigView: View {
	...
	@State private var enabledFuels: Set<AviationFuelType> = Set(AviationFuelType.defaultEnabled)
	...
	private func loadFromSettings() {
		...
		enabledFuels = Set(s.enabledFuelTypes)
	}

	private func toggle(_ fuel: AviationFuelType) {
	...
	private func restoreDefaults() {
		enabledFuels = Set(AviationFuelType.defaultEnabled)
	}

	private func save() {
		guard let s = settings else { return }
		// Keep catalog declaration order within each category rather than Set's arbitrary order.
		s.enabledFuelTypes = AviationFuelType.allCases.filter { enabledFuels.contains($0) }
		do { try modelContext.save() } catch { print("Failed to save fuel type scheme: \(error)") }
	}

	@ViewBuilder
	private func fuelRow(_ fuel: AviationFuelType) -> some View {
		Button {
			toggle(fuel)
		} label: {
			HStack {
		...
		List {
			...
			ForEach(AviationFuelType.Category.allCases, id: \.self) { category in
				Section(category.rawValue) {
					ForEach(AviationFuelType.allCases.filter { $0.category == category }) { fuel in
						fuelRow(fuel)
					}
				}
			}
