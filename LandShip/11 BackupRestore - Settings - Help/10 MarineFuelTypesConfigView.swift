// MarineFuelTypesConfigView.swift
//
// Lets the user choose which fuel types appear in the Fuel Type picker on Edit
// Vehicle / Edit Fuel Log. Only relevant to the marine vertical, which spans
// everything from ethanol-free marine gasoline to bunkered residual fuel oil.
// Saved to the CloudKit-synced Settings1 singleton, so the selection follows
// the user across devices — mirrors FuelTypesConfigView (aviation) and the
// dashboard card scheme in DashboardConfigView.

import SwiftUI
import SwiftData

struct MarineFuelTypesConfigView: View {
	...
	@State private var enabledFuels: Set<MarineFuelType> = Set(MarineFuelType.defaultEnabled)
	...
	private func loadFromSettings() {
		...
		enabledFuels = Set(s.enabledMarineFuelTypes)
	}

	private func toggle(_ fuel: MarineFuelType) {
	...
	private func restoreDefaults() {
		enabledFuels = Set(MarineFuelType.defaultEnabled)
	}

	private func save() {
		guard let s = settings else { return }
		// Keep catalog declaration order within each category rather than Set's arbitrary order.
		s.enabledMarineFuelTypes = MarineFuelType.allCases.filter { enabledFuels.contains($0) }
		do { try modelContext.save() } catch { print("Failed to save marine fuel type scheme: \(error)") }
	}

	@ViewBuilder
	private func fuelRow(_ fuel: MarineFuelType) -> some View {
		Button {
			toggle(fuel)
		} label: {
			HStack {
		...
		List {
			...
			ForEach(MarineFuelType.Category.allCases, id: \.self) { category in
				Section(category.rawValue) {
					ForEach(MarineFuelType.allCases.filter { $0.category == category }) { fuel in
						fuelRow(fuel)
					}
				}
			}
...
#Preview {
	...
	return MarineFuelTypesConfigView()
