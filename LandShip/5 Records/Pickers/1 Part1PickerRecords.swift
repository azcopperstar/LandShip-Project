...
struct Part1PickerRecords: View {
	...
	var body: some View {
		...
		InnerViewRecords_Part1(RequestingModel: RequestingModel, RequestingData: $RequestingData)
	}
}

struct InnerViewRecords_Part1: View {
	...
	var body: some View {
		...
		.onAppear(){
			selectedPicker = RequestingData.part1 // presets picker item
		}
		.onChange(of: selectedPicker) {oldValue, newValue in
					RequestingData.part1 = "\(String(describing: selectedPicker))" // returns picker choice back to view
