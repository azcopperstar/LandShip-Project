...
struct Part5PickerRecords: View {
	...
	var body: some View {
		...
		InnerViewRecords_Part5(RequestingModel: RequestingModel, RequestingData: $RequestingData)
	}
}

struct InnerViewRecords_Part5: View {
	...
	var body: some View {
		...
		.onAppear(){
			selectedPicker = RequestingData.part5 // presets picker item
		}
		.onChange(of: selectedPicker) {oldValue, newValue in
					RequestingData.part5 = "\(String(describing: selectedPicker))" // returns picker choice back to view
