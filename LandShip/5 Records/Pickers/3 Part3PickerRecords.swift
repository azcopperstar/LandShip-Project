...
struct Part3PickerRecords: View {
	...
	var body: some View {
		...
		InnerViewRecords_Part3(RequestingModel: RequestingModel, RequestingData: $RequestingData)
	}
}

struct InnerViewRecords_Part3: View {
	...
	var body: some View {
		...
		.onAppear(){
			selectedPicker = RequestingData.part3 // presets picker item
		}
		.onChange(of: selectedPicker) {oldValue, newValue in
					RequestingData.part3 = "\(String(describing: selectedPicker))" // returns picker choice back to view
