...
struct Part2PickerRecords: View {
	...
	var body: some View {
		...
		InnerViewRecords_Part2(RequestingModel: RequestingModel, RequestingData: $RequestingData)
	}
}

struct InnerViewRecords_Part2: View {
	...
	var body: some View {
		...
		.onAppear(){
			selectedPicker = RequestingData.part2 // presets picker item
		}
		.onChange(of: selectedPicker) {oldValue, newValue in
					RequestingData.part2 = "\(String(describing: selectedPicker))" // returns picker choice back to view
