...
struct VehiclePickerFuelLog: View {
	@Query var RequestingModel: [Vehicle8] // model containing the picker requesting data
	@Binding var RequestingData: FuelLog1 // model of data requested by picker, used to track change in picker

	var body: some View {
		// send systems model and mxItems model to innerview to load picker
		InnerViewFuelLog_Vehicle(RequestingModel: RequestingModel, RequestingData: $RequestingData)
	}
}

struct InnerViewFuelLog_Vehicle: View {
	let RequestingModel: [Vehicle8]
	@Binding var RequestingData: FuelLog1
