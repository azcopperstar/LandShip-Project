...
struct pdfReportAdditions: View {
	...
	@Query private var dataSet: [Additions]
...
//	@Binding var trackVehicleSelected: String
	...
	private var dataSetKey: String {
		let sorted = dataSet.sorted { (lhs: Additions, rhs: Additions) -> Bool in
	...
	init(trackVehicleSelected: String) {
		...
		let sortDescriptors: [SortDescriptor<Additions>] = [
		...
		if vehicleId != "All Vehicles" {
			...
			let id = vehicleId
			let predicate: Predicate<Additions> = #Predicate { $0.vehicleId == id }
			self._dataSet = Query<Additions, [Additions]>(
				filter: predicate,
	...
	private func generateAndShowPDF() {
        ...
        if let doc = PDFDocument(data: pdfData) {
            ...
            lastPDFHash = hash
            _ = savePDF(data: pdfData, fileName: "Additions")
        }
	}
	...
	func valuesForRecord(_ record: Additions) -> [String] {
		...
		let cost = functions.formatCurrency(dollars: max(0, record.itemCost))
		let notes = record.itemNotes

		return [date, title, description, notes, vendor, cost, miles, imagePlaceholder]
	}
	...
	func generatePDFWithTable(units: [String]) -> Data? {
		...
		let headerRows: [[String]] = [
			["DATE", "IMPROVEMENT ITEM", "DESCRIPTION", "NOTES", "VENDOR", "COST", "MILES", "IMAGE"]
		]
		...
		let reportTitle = "Improvements and Additions"
	...
	func computeRowHeight(for record: Additions, columnWidths: [CGFloat], minRowHeight: CGFloat) -> CGFloat {
	...
	// macOS variant converts to Quartz coordinates; iOS draws directly.
	func drawTableRow(record: Additions, at origin: CGPoint, columnWidths: [CGFloat], rowHeight: CGFloat, rowIndex: Int, pageHeight: CGFloat? = nil) {
		let values = valuesForRecord(record)
		let titleIndex = 1
		let descriptionIndex = 2
		let notesIndex = 3
	...
	private func saveAsPDF() {
		...
		let dateStamp = DateFormatter.localizedString(from: Date(), dateStyle: .short, timeStyle: .short)
			...
			.replacingOccurrences(of: ":", with: "-")
			.replacingOccurrences(of: ",", with: "")
		panel.nameFieldStringValue = "Additions - \(vehicleTitle) - \(dateStamp).pdf"
	...
	private func sharePDFiOS() {
		...
		let dateStamp = DateFormatter.localizedString(from: Date(), dateStyle: .short, timeStyle: .short)
			...
			.replacingOccurrences(of: ":", with: "-")
			.replacingOccurrences(of: ",", with: "")
		let fileName = "Additions - \(vehicleTitle) - \(dateStamp).pdf"
...
#Preview {
    ...
    let container = try! ModelContainer(for: Additions.self, Settings1.self, configurations: config)

    // Seed sample additions data for visual verification of layout, grouping, and totals.
    let context = container.mainContext
    func makeAddition(category: String, subCategory: String, daysAgo: Int, vehicleId: String, name: String, cost: Float, notes: String) -> Additions {
    ...
    let samples: [Additions] = [
        makeAddition(category: "Engine", subCategory: "Oil", daysAgo: 0, vehicleId: "Vehicle A", name: "Oil Change", cost: 120, notes: "Next change in 3 months."),
        makeAddition(category: "Engine", subCategory: "Oil", daysAgo: 10, vehicleId: "Vehicle A", name: "Oil Filter", cost: 18.5, notes: "OEM filter."),
        makeAddition(category: "Engine", subCategory: "Cooling", daysAgo: 20, vehicleId: "Vehicle A", name: "Coolant Flush", cost: 95, notes: "Replaced coolant."),
        makeAddition(category: "Brakes", subCategory: "Front", daysAgo: 15, vehicleId: "Vehicle A", name: "Front Pads", cost: 210, notes: "Resurfaced rotors."),
        makeAddition(category: "Brakes", subCategory: "Rear", daysAgo: 18, vehicleId: "Vehicle A", name: "Rear Pads", cost: 180, notes: "New pads."),
        makeAddition(category: "Electrical", subCategory: "Battery", daysAgo: 5, vehicleId: "Vehicle B", name: "Battery Replacement", cost: 170, notes: "Starts faster."),
        makeAddition(category: "Electrical", subCategory: "Lighting", daysAgo: 2, vehicleId: "Vehicle B", name: "Headlight Bulb", cost: 25, notes: "Left bulb replaced."),
    ]
